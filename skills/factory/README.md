...
Standalone skill: autonomous software factory for **one Linear ticket**. Plan → blind critique → implement → stacked PRs → invoke the standalone Babysit skill → merge on your green-light.
...
```mermaid
flowchart LR
  ticket --> plan --> critique --> implement --> prs --> babysit["invoke /babysit"] --> greelight[green-light] --> merge
```
...
8. Automatically invokes the standalone [Babysit](../babysit/README.md) skill
9. Babysit loops on CI + review, stops for your green-light, then merges bottom-up

## What you need

- **Linear MCP**
- **GitHub CLI (`gh`) 2.90.0+** with `gh extension install github/gh-stack`
- The standalone Babysit skill: `npx skills add chroline/oliver/skills/babysit`
- Stacked PRs enabled on the repo (or Factory falls back to chained plain PRs)

## Related

- Single-ticket PR follow-through: [Babysit](../babysit/README.md)
