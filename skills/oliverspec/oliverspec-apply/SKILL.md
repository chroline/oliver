...
**TDD required (red → stubs typecheck → green):** for each ticket, land comprehensive failing tests first, keep the tree type-checking via stubs/fakes/interfaces, then implement until tests pass. Token smoke tests are not enough.  
...
3. **Phase 1 — tests + stubs:** write comprehensive failing tests for the ACs, then stubs/fakes/types so typecheck passes while tests remain red. Commit this phase when practical (`tests + stubs` before implementation)
...
5. Run the relevant automated tests and typecheck; both must pass before submit
6. Commit onto that branch (`gt modify -am "..."` or commit then ensure Graphite metadata is intact). Prefer history that shows phase 1 then phase 2 when practical
...
8. Return PR URL + stack parent + worktree path + test summary (phase 1/2) + residual risks (parent removes the worktree unless you already did)Parent responsibilities:
