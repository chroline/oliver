...
- `list_projects` for active OliverSpec projects
- For a relevant project: PRD + TRD via `list_documents` / `get_document`, tickets via `list_issues`
...
3. **Offer to capture when decisions are made**
   ...
   |--------------|------------------|
   | New product requirement | PRD (via `oliverspec-propose`) |
   | Requirement changed | PRD / affected tickets |
   | Design decision made | TRD (via `oliverspec-propose`) |
   | Scope changed | PRD (Goals / Non-goals) |
   | New work identified | New Linear ticket (via `oliverspec-scope`) |
   | Assumption invalidated | Relevant PRD/TRD section or ticket |
